...
class Employee extends Person {
  ...
  greet() {
    //overriding a method from Person class
    write(
      `Hello, my name is ${this.name} and I am ${this.age} years old. I am a ${this.position}.`
    );
  }
  work() {
    write(`${this.name} is working.`);
  }
  set modfyPosition(position) {
    this.position = position;
  }
  get getPosition() {
    return `${this.name} now is working as a ${this.position}.`;
  } // 'this.position;}
}
...
write('filter', 'b');
write(
  myArray.filter((element) => element % 2 == 0) + ' is filtered even numbers'
);
//find
write('find', 'b');
write(
  myArray.find((element) => element % 3 == 0) + ' is found 3 multiples numbers'
);
//findIndex
write('findIndex', 'b');
write(
  myArray.findIndex((element) => element == 5) +
    ' is the index of 5 in the array'
);
//some
write('some', 'b');
...
write('every', 'b');
write(
  myArray.every((element) => element > 2) + ' is every element > 2 in the array'
);
//map
write('map', 'b');
...
write(
  'cos -  sin -  tan -  acos -  asin -  atan -  cosh -  sinh -  tanh -  acosh -  asinh -  atanh -  hypot -  expm1 -  log1p -  log10 -  log2',
  'b'
);
...
// querySelector
write(
  'document.querySelector("h1") = ' + document.querySelector('h1').innerHTML
);
// querySelectorAll
write(
  'document.querySelectorAll("p") length is ' +
...
write(
  'document.getElementById("myID") = ' +
    ...
    ' becouse it was not found'
);
// getElementsByClassName
write(
  'document.getElementsByClassName("maincont") length is ' +
    document.getElementsByClassName('maincont').length
);
// getElementsByTagName
write(
  'document.getElementsByTagName("div") length is ' +
    document.getElementsByTagName('div').length
);
// getElementsByName
write(
  'document.getElementsByName("myNAME")  length is ' +
    document.getElementsByName('myNAME').length +
    ' becouse it was not found'
);
...
if (inputElement) {
  ...
  write(
    'inputElement.setAttribute("placeholder", "enter unsername")... placeholder is set'
  );
  // getAttribute
  write(
    'inputElement.getAttribute("placeholder") is ' +
...
write(
  'type: checkbox | radio | submit | reset | button | fileupload | text | password | number| range | hidden | email |tel | url | range | date | time | week | month | color'
);
write('', 'br');
write('', 'input', '', 'radio');
...
write(
  'classList.replace(oldClass, newClass) to replace the old class with a new one'
);

write('element contnent propoerties:', 'b');

write(
  'element.textContent: displays the text content of the element and its children'
);
write(
  'element.innerHTML: displays the HTML content of the element and its children'
);
write(
  'element.outerHTML: displays the HTML content of the element and its children'
);
...
write(
  'appendChild, removeChild, replaceChild, insertBefore, hasChildNodes',
  'b'
);
...
// previousSibling
write(
  'previousSibling is "' + secondSilibing.previousSibling.textContent + '"'
);

// nextElementSibling - only returns elements
write(
  'nextElementSibling is "' + firstsilibing.nextElementSibling.textContent + '"'
);

// previousElementSibling
write(
  'previousElementSibling is "' +
    secondSilibing.previousElementSibling.textContent +
    '"'
);
...
// closest
write(
  'header\'s closest DIV classname is "' + header.closest('div').className + '"'
);
